...
function displayItems(e){
    ...
    .then(response =>{
        ...
        data.posts.forEach( item=>{
            ...
            const rating = document.querySelector('.search-item__rating-cover__gold');
            ...
            if(item.description.length > 350){
                ...
                console.log(item.description.slice(300));
                txtBox.insertAdjacentHTML('beforeend', '<a href="#" class=" search-item__title__txt__btn" >more</a>')
                const showMoreBtn = document.querySelector('.search-item__title__txt__btn');
                showMoreBtn.addEventListener( 'click', showTxt);
                txtBox.insertAdjacentHTML('afterend', '<p class="search-item__title__txt__hidden" style="display: none">'+txtElse+'</p>')
